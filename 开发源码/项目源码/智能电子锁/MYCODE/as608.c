...
{   
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;//输入下拉模式
...
{
	MYUSART_SendData(AS608Addr>>24); //发送地址24-31位
	MYUSART_SendData(AS608Addr>>16); //发送地址16-23位
	MYUSART_SendData(AS608Addr>>8);  //发送地址8-16位
	MYUSART_SendData(AS608Addr);     //发送地址0-7位
}
//发送包标识,
static void SendFlag(u8 newflag)
{
	MYUSART_SendData(newflag);
}
//发送包长度
